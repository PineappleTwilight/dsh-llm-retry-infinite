...
export function renderTerminalStatus(state) {
  ...
  const delayFormatted = state.delayFormatted ?? formatDelay(state.delayMs ?? 0);
  return `${emoji} Retrying #${state.retry} — ${label}${codeStr}, waiting ${delayFormatted} (${remaining} remaining)`;
}
...
export function renderHTMLIndicator(state) {
  ...
  const codeStr = state.statusCode
    ...
    : "";
  const remaining = state.remainingFormatted ?? "0s";
  const progressPct = state.delayMs && state.remainingMs !== undefined
    ? Math.max(0, Math.min(100, (state.remainingMs / state.delayMs) * 100))
    : 0;
  ...
  <div class="retry-indicator-body">
    ...
    <div class="retry-countdown">
      <div class="retry-countdown-bar">
        <div class="retry-countdown-fill" style="--retry-progress: ${progressPct}%"></div>
      </div>
      <span class="retry-countdown-text">${remaining} remaining</span>
    </div>
    <div class="retry-cumulative">
      Total wait: ${formatDelay(state.cumulativeWaitMs ?? 0)}
    </div>
  </div>
</div>`.trim();
}
...
export function renderMarkdownStatus(state) {
  ...
  const remaining = state.remainingFormatted ?? "0s";
  const delayFormatted = state.delayFormatted ?? formatDelay(state.delayMs ?? 0);
  return `${emoji} **Retrying #${state.retry}** — ${label}${codeStr} · waiting ${delayFormatted} · ${remaining} left`;
}
